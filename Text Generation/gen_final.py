...
from prompts import gen_one_shot_prompt 
import pandas as pd
import os
...
def gen(style):
    OUTPUT_CSV_PATH_A = "gen_final_B_" + str(style) + ".csv"
    
    # Initialize Model A's engine
    # This will reserve a large block of VRAM
    llm_A = LLM(model="meta-llama/Llama-3.1-8B-Instruct", download_dir="/work/shiyxu/hf_cache",dtype="auto")
    ...
    # Load your data
    df_A = pd.read_csv(f"../Filter_Triples_from_KG/relations_B_{style}_new.csv")

    print(f" - Style: {style}")
    prompts_list_A = df_A.apply(
        lambda row: gen_one_shot_prompt(input_triple=row["relations"], style=style),
        axis=1
    ).tolist()
...
#     OUTPUT_CSV_PATH_A = "gen_A_50_" + str(model_name) + "_one_shot.csv"
...
#         print(f" - Style: {style}")
#         prompts_list_A = df_A.apply(
#             lambda row: gen_one_shot_prompt(input_triple=row["triples"], style=style),
#             axis=1
#         ).tolist()
