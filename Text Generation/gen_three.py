...
from prompts import gen_three_shot_prompt 
import pandas as pd
import os
...
def gen(model_path_A):
    OUTPUT_CSV_PATH_A = "gen_A_50_" + str(model_path_A) + "_three_shot.csv"
    ...
    for style in ["academic", "oral", "twitter"]:
        ...
        prompts_list_A = df_A.apply(
            lambda row: gen_three_shot_prompt(input_triple=row["triples"], style=style),
            axis=1
        ).tolist()
...
#     OUTPUT_CSV_PATH_A = "gen_A_50_" + str(model_name) + "_three_shot.csv"
